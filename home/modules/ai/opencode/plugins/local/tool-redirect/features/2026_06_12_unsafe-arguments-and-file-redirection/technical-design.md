...
A long flag with an attached value is normalized to its flag name: `--output=file.txt` becomes `{ kind: "flag", name: "--output" }`. The value is discarded because the current rules only need to know that the flag is present. Other attached forms, such as `-ofile.txt`, `-i.bak`, and combined short flags, are left unrecognized for now.

### Redirects

A redirect has the literal shell operator and a target. Output operators and the read-write `<>` operator are modelled. Plain input redirections (`<`, `<&`) never write a file, so the feature ignores them.
...
The first implementation intentionally leaves these cases unsupported:

- Attached or combined short flags, such as `sort -ofile`, `sed -i.bak`, and `sed -ni`.
- Dynamic programs, flags, and redirect targets, such as `$program file`, `sed "$flag" file`, and `cmd > "$output"`.
- Commands hidden behind wrappers such as `env`, `command`, and `sudo`.
- Redirects attached to compound constructs, such as `(cmd) > file` or `{ cmd; } > file`, because they do not belong to one concrete flattened command.
- Descriptor closing and non-standard descriptors, such as `2>&-` and `3>&4`.
- Process substitutions, including redirect targets such as `> >(cmd)`.
- State-changing syntax inside program strings, such as `awk`'s `system()` and `sed`'s `w` command.
- Alternate spellings of `/dev/null`, including symlinks and paths containing expansions.
